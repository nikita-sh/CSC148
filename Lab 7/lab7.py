...
def list_internal(t: Tree) -> list:
    ...
    if not t.children:
        ...
        return sum([list_internal(i) for i in t.children], [t.value])
...
def count_internal(t: Tree) -> int:
    ...
    if not t.children:
        ...
        return 1 + sum([count_internal(i) for i in t.children])
...
def count_leaves(t: Tree) -> int:
    ...
    if not t.children:
        ...
        return sum([count_leaves(i) for i in t.children])
...
def sum_internal(t: Tree) -> int:
    ...
    if not t.children:
        ...
        return t.value + sum([sum_internal(i) for i in t.children])
...
def sum_leaves(t: Tree) -> int:
    ...
    if not t.children:
        ...
        return sum([sum_leaves(i) for i in t.children])
...
def arity(t: Tree) -> int:
    ...
    if not t.children:
        ...
        return max(len(t.children), max([arity(i) for i in t.children]))
...
def contains_test_passer(t: Tree, test: Callable[[Any], bool]) -> bool:
    ...
    if not t.children:
        ...
        val_bool = test(t.value)
        return True if val_bool else any([contains_test_passer(i, test)
                                          for i in t.children])
...
def list_if(t: Tree, p: Callable[[Any], bool]) -> list:
    ...
    if not t.children:
        ...
        return [t.value] + sum([list_if(x, p) for x in t.children], []) \
            if p(t.value) else sum([list_if(x, p) for x in t.children], [])
